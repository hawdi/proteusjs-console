...
internals.utility = {
  ...
  //format log level
  formatLogLevel(logLevel, settings) {
    ...
    if (settings.color) {
      const color = logLevelColor[level] || 34; //default blue
  ...
  formatServerLog(event, settings) {
    ...
    const object = `${event.object}:${event.event}`
  ...
  formatServerError(event, settings) {

    const output = `message: ${event.errorMessage} stack: ${event.errorStack}`;
    const object = `${event.object}:${event.event}`
  ...
  formatDBEnd(event, settings) {
    ...
    const level = internals.utility.formatLogLevel('info', settings);
  ...
  formatDBQueryError(event, settings) {
    ...
    const level = internals.utility.formatLogLevel('error', settings);
    const output = `${level}: [ ${event.message} ]`;
    const object = `${event.object}:${event.event}`;
  ...
  formatHttpRequest(event, settings) {
    ...
    const output = `${method} ${event.href}`;
    const object = `${event.object}:${event.event}`;
  ...
  formatHttpResponse(event, settings) {
    ...
    const output = `${method} ${event.href} (${statusCode}|${event.statusMessage})`;
    const object = `${event.object}:${event.event}`;
  ...
  _transform(data, enc, next) {
    ...
    if (objName === 'server') {
      ...
      if(eventName === 'response') {
        ...
        return next(null, internals.utility.formatServerError(data, this._settings));
      }
    ...
    next(null, 'proteusjs-console : no event found!');
  }
